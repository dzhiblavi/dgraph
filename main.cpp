...
void main_window_loop(dgl::Window& wd) {
    ...
    try {
        ...
        dgl::GpProg def_prog(def_vert, def_frag);
...
int main(int argc, char** argv) {
    tmnl::cargs c(argc, argv);
    tmnl::logger lout(std::cout);
    path = std::filesystem::path(c.getStr("path")).parent_path() / "../";

    try {
        dgl::Context ctx;

        dgl::Window w1(ctx, {1600, 1200, "test window 1", nullptr, nullptr}, 
            [&w1] {
                main_window_loop(w1);
            });

        glfwSetKeyCallback(w1.native_handle(), key_callback);
        glfwSetInputMode(w1.native_handle(), GLFW_CURSOR, GLFW_CURSOR_DISABLED);
        glfwSetCursorPosCallback(w1.native_handle(), mouse_callback);
